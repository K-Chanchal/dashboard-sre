...
// Re-create the routes directly
app.get('/api/servers', async (req, res) => {
    try {
        const db = getDbPool();
        const [rows] = await db.query('SELECT * FROM server_status ORDER BY created_at DESC');
        res.json(rows);
    } catch (error) {
...
app.get('/api/servers/status/:status', async (req, res) => {
    try {
        ...
        const [rows] = await db.query(
            'SELECT * FROM server_status WHERE status = ? ORDER BY created_at DESC',
            [status]
        );
        res.json(rows);
    } catch (error) {
...
app.get('/api/servers/:id', async (req, res) => {
    try {
        const db = getDbPool();
        const { id } = req.params;
        const [rows] = await db.query('SELECT * FROM server_status WHERE id = ?', [id]);
...
app.put('/api/servers/:id', async (req, res) => {
    try {
        ...
        await db.query(
            'UPDATE server_status SET status = ?, message = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
            [status, message, id]
        );
